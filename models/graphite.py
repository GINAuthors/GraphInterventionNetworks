...
from models.building_blocks.utils import get_optimiser_scheduler
...
class GraphITE(NeuralNetworkEstimator):
    def __init__(self, args):
        ...
        self.optimiser, self.lr_scheduler = get_optimiser_scheduler(args=args, model=self)
        self.is_multi_relational = args.gnn_multirelational

    def loss(self, prediction: Tensor, batch: Batch):
        pred_outcome, unit_features, treatment_features = prediction[0].view(-1), prediction[1], prediction[2]
        target_outcome = batch.y
        outcome_loss = F.mse_loss(input=pred_outcome, target=target_outcome)
        HSIC_regularisation = self.independence_regularisation_coeff * hsic_normalized(x=unit_features,
                                                                                       y=treatment_features)
        return outcome_loss + HSIC_regularisation

    def forward(self, batch: Batch):
        treatment_node_features, treatment_edges, covariates, batch_assignments = batch.x, batch.edge_index, batch.covariates, batch.batch
