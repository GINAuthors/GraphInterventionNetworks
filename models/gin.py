...
class GIN(nn.Module):
    def __init__(self, args: Namespace):
        ...
        self.treatment_net_opt, self.treatment_net_lr_sched = get_optimiser_scheduler(args=args,
                                                                                      model=self.treatment_net,
                                                                                      net='gnn')
        self.propensity_net_opt, self.propensity_net_lr_sched = get_optimiser_scheduler(args=args,
                                                                                        model=self.propensity_net,
                                                                                        net='propensity')
        self.como_net_opt, self.com_net_lr_sched = get_optimiser_scheduler(args=args, model=self.como_net, net='com')
        self.covariates_net_opt, self.covariates_net_lr_sched = get_optimiser_scheduler(args=args,
