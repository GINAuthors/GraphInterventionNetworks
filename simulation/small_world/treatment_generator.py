...
def generate_sw_graphs(num_graphs: int, min_nodes: int, max_nodes: int, min_neighbours: int, max_neighbours: int):
    graphs = []
    ...
    for i in tqdm(range(num_graphs)):
        ...
        prob_edge = np.random.uniform(0.1, 1.)
        graph = nx.connected_watts_strogatz_graph(n=num_node, k=k, p=prob_edge)
        degree_centralities = np.expand_dims(list(nx.algorithms.centrality.degree_centrality(graph).values()), axis=1)
